import type { Distribution, DistributionsResponse } from '@/types/cdn';
...
function generateDummyDistributions(): Distribution[] {
	...
	for (let i = 1; i <= 150; i++) {
		...
		const cacheStrategy =
			cacheStrategies[Math.floor(Math.random() * cacheStrategies.length)];
		const orgId =
		...
		distributions.push({
			...
			description: `CDN distribution for ${name} - ${
...
const allDummyData: Distribution[] = generateDummyDistributions();
